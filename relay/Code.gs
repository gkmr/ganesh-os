 ...
 *  MAIN ids" the gap report counts are mostly the relay's own pin service
 *  messages, not lost replies. So:
 *  1. inboxSelfHeal_ now needs UPDATE-COUNTER evidence: getWebhookInfo says a
 *     webhook is set, OR pending updates exist that our offset skips past, OR the
 *     last inbound update_id jumped by more than 3 over the previous one (updates
 *     consumed elsewhere). Gap-report ids alone never trigger it.
 *  2. pollInbox records the update-id jump per inbound (tg_last_update_id) and
 *     writes a mirror meta row "update-jump" when the jump exceeds 3.
 *  3. The 36h "no inbound captured" watchdog line goes to OPS, not MAIN, and
 *     says plainly that silence is normal if GK has not texted.
 *  Previous:
 *  v7.9.4 (2026-09-09) — SELF-DIAGNOSING REPLY LANE + SERVER-SIDE SUPRA.
 *  Everything in this version runs from main() on the existing 5-min trigger.
 *  GK pastes the file, deploys a NEW VERSION of the EXISTING deployment, and
 *  never has to pick a function to run.
 *  1. inboxSelfHeal_(): when sends flow but telegram-inbox.jsonl is silent and
 *     today's mirror gap report shows human message ids the relay never saw,
 *     it calls getWebhookInfo, deletes a stray webhook, resets a stuck offset,
 *     and posts ONE diagnosis line to OPS (rate-limited 6h). The 168h quiet
 *     threshold that hid the Sep 3-9 outage is now 36h.
 *  2. supraTick_(): every 6h the relay itself reads the Supra job board through
 *     the Coda API (Script Property CODA_TOKEN) and writes
 *     supra-board-latest.json + supra-board-status.txt to Claude-Fleet-State.
 *     Cloud lanes read Drive instead of POSTing to /exec; a 401/403 is written
 *     into the status file in plain words so the job-scan lane can report it.
 *  3. bootstrapDiag_(): on the first tick after this version deploys, one OPS
 *     message with version, webhook state, pending updates, offset, inbox age,
 *     supra status. Proof the paste worked, no manual function run needed.
 *  4. Gap report: OPS gaps dropped (humans do not type in OPS; the ops list was
 *     noise). MAIN gaps unchanged and now feed the self-heal.
 *  5. Content guard: still REFUSES Doc-typed queue files (writer lanes must fix
 *     themselves) but the OPS note now names the lane and the one-line fix.
 *     Set REPAIR_DOC_TYPED = true to switch to repair-and-send.
 *  Previous:
...
var WD_INBOX_QUIET_HOURS = 36;  // v7.9.4: was 168; the Sep 3-9 dead reply lane hid behind it. 36h + the gap-report signal below.
var WD_GAP_MIN_IDS = 5;         // v7.9.4: MAIN message ids the relay neither sent nor received today (v7.9.5: informational only, pins land here too)
var WD_UPDATE_JUMP = 3;         // v7.9.5: inbound update_id jump above this => updates were consumed by someone else
var WD_INBOX_HEAL_HOURS = 12;   // v7.9.4: inbox file untouched this long while gaps exist => run the self-heal
var REPAIR_DOC_TYPED = false;   // v7.9.4: false = refuse Doc-typed queue files (writer must fix); true = export as text and send anyway
var SUPRA_EVERY_HOURS = 6;      // v7.9.4: server-side Supra board read cadence
var SUPRA_DOC = 'Ao0DCca5JG';   // v7.9.4: Supra Job Board doc (verified 2026-08-26)
var SUPRA_TABLE = 'grid-wzdxbJe8SU'; // v7.9.4: "Latest Roles" table
var SUPRA_MAX_PAGES = 12;       // v7.9.4: 12 x 25 rows per tick is plenty for a 14-day window
...
function main() {
  ...
  try {
    ...
    if (timeLeft_() > 30000) { try { inboxSelfHeal_(); } catch (e) { console.error('inboxSelfHeal', e); } } // v7.9.4
    if (timeLeft_() > 60000) { try { supraTick_(); } catch (e) { console.error('supraTick', e); } }         // v7.9.4
    if (timeLeft_() > 20000) { try { bootstrapDiag_(); } catch (e) { console.error('bootstrapDiag', e); } } // v7.9.4
...
function watchdogTick_() {
  ...
  if (inboxAgeH !== null && inboxAgeH > WD_INBOX_QUIET_HOURS && slogAgeH !== null && slogAgeH < 24) {
    if (wdShouldAlert_(props, 'wd_last_inboxstale_alert', now)) {
      sendText_('ℹ️ [relay/watchdog] no inbound Telegram message in ' + inboxAgeH.toFixed(0) +
                'h while sends flow. Normal if GK has not texted the bot. The self-heal watches for real evidence ' +
                '(webhook set, skipped queue, update-counter jump) and posts a 🩺 line only then.', '', OPS);   // v7.9.5: OPS, not MAIN
...
function mirrorGapReport_() {
  ...
  var gaps = {};
  ['main'].forEach(function (c) {                                             // v7.9.4: OPS dropped, humans do not type there
  ...
  try {
    ...
    if (body.op === 'version') {
      return json_({ ok: true, version: RELAY_VERSION,
                     ...
                     supra: 'server-side every ' + SUPRA_EVERY_HOURS + 'h -> Claude-Fleet-State/supra-board-latest.json' });
    }
      ...
      if ((/\.(md|txt|tgmsg)$/i).test(n)) {
        ...
        if (why) {
          b.createFile(n + '.delivered', 'DEAD-LETTER (content guard): ' + why,
                       MimeType.PLAIN_TEXT);
          if (REPAIR_DOC_TYPED && f.getMimeType() === MimeType.GOOGLE_DOCS) {          // v7.9.4 optional repair path
            try {
              t = fixMojibake_(f.getAs(MimeType.PLAIN_TEXT).getDataAsString());   // export, no new OAuth scope
              why = null;
            } catch (eRep) { console.error('doc repair', eRep); }
          }
          if (why) {
            sendText_('🔧 [relay-guard] refused to post "' + n + '" — ' + why +
                      '. Writer lane: ' + laneFromName_(n) + '. Fix: create_file with contentMimeType text/plain ' +
                      'AND disableConversionToGoogleType:true, then read back mimeType. File dead-lettered; ' +
                      'GK did not receive it.', '', OPS);
            processed++;
            return;
          }
        }
...
// ===================== v7.9.4 INBOX SELF-HEAL =====================
/** Signature of a dead reply lane: sends are flowing, telegram-inbox.jsonl has
 *  not been written for WD_INBOX_HEAL_HOURS, and today's mirror gap report lists
 *  at least WD_GAP_MIN_IDS MAIN message ids the relay neither sent nor received
 *  (in a private chat those are the human's messages). Two silent killers are
 *  handled here without anyone opening the editor: a stray webhook (Telegram
 *  then refuses getUpdates) and a cursor that ran ahead of the queue. */
function inboxSelfHeal_() {
  var props = PropertiesService.getScriptProperties();
  var root = fleetRoot_(); if (!root) return;
  var now = Date.now();
  var inbox = newestByName_(root, 'telegram-inbox.jsonl');
  var inboxAgeH = inbox ? (now - inbox.getLastUpdated().getTime()) / 36e5 : 999;
  var gaps = todayMainGaps_();
  if (inboxAgeH < WD_INBOX_HEAL_HOURS) return;                                 // still hearing from GK: nothing to heal
  if (!wdShouldAlert_(props, 'wd_last_selfheal', now)) return;

  var diag = inboxDiag_();                                                      // read-only peek, cheap
  var jump = Number(props.getProperty('wd_update_jump') || 0);
  var skipped = (!diag.webhook_url && diag.pending > 0 && diag.first_update_id && diag.tg_offset >= diag.first_update_id);
  var evidence = !!diag.webhook_url || skipped || jump > WD_UPDATE_JUMP;
  if (!evidence) return;                                                       // v7.9.5: gap-report ids alone are not evidence (pins live there)
  props.setProperty('wd_last_selfheal', String(now));
  props.deleteProperty('wd_update_jump');
  var actions = [];
  if (diag.webhook_url) {
    var d = UrlFetchApp.fetch('https://api.telegram.org/bot' + BOT + '/deleteWebhook?drop_pending_updates=false', { muteHttpExceptions: true });
    actions.push('deleted webhook ' + diag.webhook_url.slice(0, 60) + ' (HTTP ' + d.getResponseCode() + ')');
  }
  if (!diag.webhook_url && diag.pending > 0 && diag.first_update_id && diag.tg_offset >= diag.first_update_id) {
    props.deleteProperty('tg_offset');
    actions.push('reset tg_offset ' + diag.tg_offset + ' (queue holds ' + diag.pending + ' updates from id ' + diag.first_update_id + ')');
  }
  var verdict;
  if (actions.length) verdict = 'self-heal applied: ' + actions.join('; ') + '. Next tick re-polls.';
  else if (jump > WD_UPDATE_JUMP) verdict = 'update counter jumped by ' + jump + ' between two inbound messages: ' + (jump - 1) + ' updates were consumed by another client with this bot token (a second script, a local poller, an old deployment still on a trigger). Find and stop it; nothing else may poll this bot.';
  else verdict = 'queue holds ' + diag.pending + ' updates but the poll returns none: ' + (diag.error || 'unknown') + '.';
  var text = '🩺 [relay/self-heal] evidence of lost replies (inbox silent ' + inboxAgeH.toFixed(0) + 'h; gap-report ids today=' + gaps.length + ', informational). ' +
             'webhook=' + (diag.webhook_url ? 'SET' : 'none') + ' pending=' + diag.pending + ' offset=' + diag.tg_offset +
             (diag.error ? ' err=' + diag.error : '') + '. ' + verdict;
  sendText_(text, '', OPS);
  mirrorAppend_([{ ts: new Date().toISOString(), dir: 'meta', kind: 'self-heal', inbox_age_h: Number(inboxAgeH.toFixed(1)),
                   gaps: gaps.length, webhook: !!diag.webhook_url, pending: diag.pending, offset: diag.tg_offset,
                   actions: actions, error: diag.error || null }]);
}

/** Read-only look at the bot's inbound state. Consumes nothing. */
function inboxDiag_() {
  var props = PropertiesService.getScriptProperties();
  var out = { webhook_url: '', pending: 0, tg_offset: Number(props.getProperty('tg_offset') || 0), first_update_id: null, error: '' };
  try {
    var w = UrlFetchApp.fetch('https://api.telegram.org/bot' + BOT + '/getWebhookInfo', { muteHttpExceptions: true });
    var wj = JSON.parse(w.getContentText());
    if (wj.ok) { out.webhook_url = wj.result.url || ''; out.pending = Number(wj.result.pending_update_count || 0); if (wj.result.last_error_message) out.error = String(wj.result.last_error_message).slice(0, 120); }
    else out.error = 'getWebhookInfo ' + (wj.description || w.getResponseCode());
  } catch (e) { out.error = 'getWebhookInfo threw ' + String(e).slice(0, 100); }
  if (!out.webhook_url) {
    try {   // peek at the oldest retained update WITHOUT moving the cursor (offset omitted, limit 1, timeout 0)
      var p = UrlFetchApp.fetch('https://api.telegram.org/bot' + BOT + '/getUpdates?timeout=0&limit=1', { muteHttpExceptions: true });
      var pj = JSON.parse(p.getContentText());
      if (pj.ok && pj.result.length) out.first_update_id = pj.result[0].update_id;
      else if (!pj.ok) out.error = (out.error ? out.error + '; ' : '') + 'getUpdates ' + String(pj.description || p.getResponseCode()).slice(0, 100);
    } catch (e2) { out.error = (out.error ? out.error + '; ' : '') + 'getUpdates threw ' + String(e2).slice(0, 80); }
  }
  return out;
}

/** MAIN message ids from the newest gap-report row in today's mirror. */
function todayMainGaps_() {
  var f = mirrorFile_(false); if (!f) return [];
  var rows = f.getBlob().getDataAsString().split('\n').filter(String), last = null;
  for (var i = rows.length - 1; i >= 0; i--) {
    if (rows[i].indexOf('"gap-report"') === -1) continue;
    try { last = JSON.parse(rows[i]); break; } catch (e) {}
  }
  return (last && last.gaps && last.gaps.main) ? last.gaps.main : [];
}

/** Best-effort lane name from a queue filename: "food-log-2026-09-05-snack.tgmsg" -> "food-log". */
function laneFromName_(n) {
  var m = String(n).match(/^([a-z][a-z0-9]*(?:-[a-z][a-z0-9]*)*?)-\d{4}/i);
  return m ? m[1] : String(n).replace(/\.[^.]+$/, '');
}

// ===================== v7.9.4 SERVER-SIDE SUPRA BOARD =====================
/** Every SUPRA_EVERY_HOURS the relay reads the Supra job board through the Coda
 *  API and writes the rows to Claude-Fleet-State/supra-board-latest.json plus a
 *  one-line supra-board-status.txt. Cloud lanes read Drive; they no longer need
 *  to POST to /exec for the board (that path broke on Sep 5-9: error pages,
 *  empty responses, egress). A 401/403 is explained in the status file. */
function supraTick_() {
  var props = PropertiesService.getScriptProperties();
  var last = Number(props.getProperty('supra_last') || 0);
  if (Date.now() - last < SUPRA_EVERY_HOURS * 36e5) return;
  props.setProperty('supra_last', String(Date.now()));
  var root = fleetRoot_(); if (!root) return;
  var tok = props.getProperty('CODA_TOKEN');
  var status = { fetched_at: new Date().toISOString(), ok: false, status: 0, count: 0, pages: 0, error: '' };
  var rows = [];
  if (!tok) {
    status.error = 'no CODA_TOKEN Script Property. Create an API token at coda.io/account (or docs.superhuman.com account settings) with access to the Supra Job Board, then Project Settings > Script Properties > CODA_TOKEN.';
  } else {
    var base = 'https://coda.io/apis/v1', pageToken = '';
    for (var p = 0; p < SUPRA_MAX_PAGES && timeLeft_() > 45000; p++) {
      var url = base + '/docs/' + SUPRA_DOC + '/tables/' + SUPRA_TABLE + '/rows?useColumnNames=true&limit=25' + (pageToken ? '&pageToken=' + encodeURIComponent(pageToken) : '');
      var r = UrlFetchApp.fetch(url, { muteHttpExceptions: true, followRedirects: true, headers: { 'Authorization': 'Bearer ' + tok, 'Accept': 'application/json' } });
      status.status = r.getResponseCode(); status.pages = p + 1;
      if (status.status >= 400) {
        var bodyTxt = r.getContentText().slice(0, 300);
        if (status.status === 401) status.error = 'HTTP 401: CODA_TOKEN is invalid or expired. Re-create the token and update the Script Property.';
        else if (status.status === 403) status.error = 'HTTP 403: the token is valid but has no access to doc ' + SUPRA_DOC + '. Either the token was created with a doc restriction that excludes the Supra board, or the board is no longer shared with this account. Open the board while signed in, then create an UNRESTRICTED token. Coda said: ' + bodyTxt.replace(/\s+/g, ' ');
        else if (status.status === 404) status.error = 'HTTP 404: doc or table id moved. Resolve via /resolveBrowserLink?url=<board url> and update SUPRA_DOC / SUPRA_TABLE.';
        else if (status.status === 429) status.error = 'HTTP 429: Coda rate limit; next tick retries.';
        else status.error = 'HTTP ' + status.status + ': ' + bodyTxt.replace(/\s+/g, ' ');
        break;
      }
      var j; try { j = JSON.parse(r.getContentText()); } catch (eJ) { status.error = 'non-JSON body from Coda'; break; }
      (j.items || []).forEach(function (it) {
        var v = it.values || {};
        rows.push({ id: it.id, title: v['Actual Role Name'] || v['Job Title'] || '', company: v['Company Name'] || '',
                    seniority: v['Seniority'] || '', location: v['Location'] || '', link: v['App Link'] || '',
                    referral: v['Supra Referrals'] || '', date_added: v['Date Added'] || '', badge: v['Badge'] || '',
                    overview: String(v['Role Overview'] || '').slice(0, 600) });
      });
      pageToken = j.nextPageToken || '';
      if (!pageToken) { status.ok = true; break; }
      status.ok = true;                                 // partial pages still count as a good read
    }
  }
  status.count = rows.length;
  var payload = JSON.stringify({ status: status, rows: rows });
  writeFleetFile_(root, 'supra-board-latest.json', payload, MimeType.PLAIN_TEXT);
  writeFleetFile_(root, 'supra-board-status.txt',
    (status.ok ? 'OK' : 'FAIL') + ' | ' + status.fetched_at + ' | HTTP ' + status.status + ' | ' + status.count + ' rows, ' + status.pages + ' pages' + (status.error ? ' | ' + status.error : '') + '\n',
    MimeType.PLAIN_TEXT);
  var prevErr = props.getProperty('supra_last_error') || '';
  if (!status.ok && status.error !== prevErr) {            // one OPS line per distinct failure, never a repeat
    sendText_('🔧 [relay/supra] board read failed - ' + status.error.slice(0, 400), '', OPS);
  }
  props.setProperty('supra_last_error', status.ok ? '' : status.error);
}

/** Overwrite-in-place (keeps the file id stable) or create. */
function writeFleetFile_(root, name, content, mime) {
  var f = newestByName_(root, name);
  if (f) f.setContent(content); else root.createFile(name, content, mime);
}

// ===================== v7.9.4 BOOTSTRAP DIAGNOSIS =====================
/** First tick after a new version deploys: one OPS message proving the paste
 *  worked and showing the inbound state. Runs once per RELAY_VERSION. */
function bootstrapDiag_() {
  ...
  sendText_('🛠 [relay] v' + RELAY_VERSION + ' live on the trigger. Inbound: webhook=' + (d.webhook_url ? 'SET ' + d.webhook_url.slice(0, 50) : 'none') +
            ...
            '. Self-heal fires only on evidence (webhook set, skipped queue, update-counter jump > ' + WD_UPDATE_JUMP + ').', '', OPS);
}
...
function pollInbox() {
  ...
  var prevLast = Number(props.getProperty('tg_last_update_id') || 0);          // v7.9.5
  data.result.forEach(function (u) {
    if (prevLast && u.update_id - prevLast > WD_UPDATE_JUMP) {                  // v7.9.5: gap in the counter = consumed elsewhere
      try { mirrorAppend_([{ ts: new Date().toISOString(), dir: 'meta', kind: 'update-jump', from: prevLast, to: u.update_id, jump: u.update_id - prevLast }]); } catch (eJ) {}
      props.setProperty('wd_update_jump', String(u.update_id - prevLast));
    }
    prevLast = u.update_id;
  ...
  props.setProperty('tg_last_update_id', String(offset));                       // v7.9.5
