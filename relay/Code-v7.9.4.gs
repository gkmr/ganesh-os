...
var RELAY_VERSION = '7.9.4'; // v7.9 reply-link footer + v7.9.1 drive_put + v7.9.2 recency_index/drive_get + v7.9.3 op:version (2026-09-03)
...
var WD_GAP_MIN_IDS = 5;         // v7.9.4: MAIN message ids the relay neither sent nor received today => humans are typing and nothing lands
...
function inboxSelfHeal_() {
  ...
  if (inboxAgeH < WD_INBOX_HEAL_HOURS || gaps.length < WD_GAP_MIN_IDS) return;
  ...
  else if (diag.pending === 0) verdict = 'Telegram queue is EMPTY while ' + gaps.length + ' human message ids are unaccounted today: another client is consuming getUpdates with this bot token (a second script, a local poller, or an old deployment still on a trigger). Find and stop it; nothing else may poll this bot.';
  else verdict = 'queue holds ' + diag.pending + ' updates but the poll returns none: ' + (diag.error || 'unknown') + '.';
  var text = '🩺 [relay/self-heal] reply lane silent ' + inboxAgeH.toFixed(0) + 'h, ' + gaps.length +
             ' MAIN ids unaccounted today (' + gaps.slice(0, 6).join(',') + (gaps.length > 6 ? '…' : '') + '). ' +
...
function bootstrapDiag_() {
  ...
  sendText_('🛠 [relay] v' + RELAY_VERSION + ' live on the trigger. Inbound: webhook=' + (d.webhook_url ? 'SET ' + d.webhook_url.slice(0, 50) : 'none') +
            ...
            '. Self-heal arms when inbox age > ' + WD_INBOX_HEAL_HOURS + 'h and ids > ' + WD_GAP_MIN_IDS + '.', '', OPS);
}
