...
**Status:** Accepted · 2026-07-13 · in its own file: [`adr-17-store-cutover-two-lane.md`](adr-17-store-cutover-two-lane.md) · extends [ADR-14](adr-14-one-store-triple-channel.md) and the two-engine law (ADR-15, above) · applies the [ADR-13](adr-13-channel-strategy.md) pause and re-entry ladder.

**Summary.** The one store moved. The task corpus migrated from the on-device reminders store to a cloud-API task store in one manifested pass: priorities, tags, native recurrences, and checklists mapped explicitly; every item carrying a backref to its source record; 33 ambiguous rows held in a REVIEW queue instead of guessed; the source store untouched. Per the two-engine law the old engine coexists read-only - one health carve-out lane plus a verdict-gated retirement check-in three days out (RETIRE / NOT YET / KEEP MIRROR), after a verified create-reply-apply round-trip on the new store. The end state is two lanes: the hosted lane owns discovery, triage, and the ask loop; the local lane owns channel adaptation and the carve-outs; the single-writer fence survived the split unchanged. Two patterns are named in the file: the GV-class reader (a channel whose platform risk attaches to sending re-enters read-only permanently - reading is the classification, not a probation step) and the injection pause (agents refusing an in-band redirect of personal-message delivery until first-party confirmation - the safety architecture working as designed). Full record: [`adr-17-store-cutover-two-lane.md`](adr-17-store-cutover-two-lane.md).

**ADR-17 follow-through** (2026-07-13): the architecture map was updated to the post-cutover end state - one cloud store, two lanes, the heartbeat pair, gate-and-worker model tiering, and the token-diet patterns, in [`architecture-map.md`](architecture-map.md).

---

## ADR-18 - The signal law, the two-channel split, and identity-by-id

**Status:** Accepted · 2026-07-23 · extends [ADR-12](#adr-12---taxonomy-rename-the-delivery--notification-contract-and-fleet-health-hardening) and the delivery law ([`delivery-law.md`](delivery-law.md)).

**Context.** Two days of live all-cloud traffic drew an outside reader's verdict: noisy, not actionable. Separately, a same-name duplicate of the store's root folder made every by-name lookup nondeterministic (frozen sent-log, misfiled health payloads), and a migration chat nagged daily about a move it structurally could not complete.

**Decision.** Three laws. (1) **Signal law:** silence by clock stays forbidden; silence by no-new-content becomes the default for status-class agents - a silent run still writes state and a run marker. The operator's main channel carries only act-on content; a second ops channel carries the engine room, routed by an explicit flag and recorded per-send in an append-only sent-log. (2) **Identity by id:** the relay pins the store root by immutable folder id, never by name; the duplicate was quarantined, its strays rescued. (3) **Habits, not tasks:** device-time rituals move to the task platform's native habit objects and are excluded from triage, boards, and overdue counts by standing law; the migration chat retires.

**Consequences.** The feed's signal-to-noise is governed by content, not schedule; debugging moved from chat into files (sent-log + run markers), so quiet no longer means blind. By-name store resolution is retired as a failure class rather than patched. One standing nag class deleted with the category error that caused it. Cost, named: two channels to watch instead of one - accepted because the ops channel is pull-based by design ("check when curious, or when main pings a failure").

---

## ADR-19 - Relay reflexes, the family tree, and the quota diet

**Status:** Accepted · 2026-07-27 · extends [ADR-18](#adr-18---the-signal-law-the-two-channel-split-and-identity-by-id) and the delivery law ([`delivery-law.md`](delivery-law.md)).

**Context.** A week of live traffic exposed the relay as the plane's soft spot: overlapping timer runs double-posted a digest sixteen seconds apart, execution-ceiling kills left the outbox unarchived so every tick re-scanned history, a document-format file posted as junk, and a post-outage backlog replayed stale anchors. Separately, thirty-plus task names had no scheme (the operator could not trace a push to its task), and a spend-limit incident ran the fleet dark for a day - triggers fired, nothing wrote.
