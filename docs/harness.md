...
| **Tools & connectors** | File tools, a sandboxed shell, and connectors (Reminders, Calendar ×6, Notes, Slack, Gmail, WhatsApp, iMessage, Google Voice, Granola + Krisp). | A surface check degrades gracefully when a connector is down and queues write-intents for the next full run. |
