...
STEP 6 - DELIVER per the format contract: [usps] tag, line-1 counts + "N need you", action items first each with a D# handle, FYIs compressed, ad mailers on one tail line. Write a manifest line per repliable piece. Send chat + one SMS + the vault file. Verbatim OCR and images stay in the channel archive; the SMS carries only a summary and the sender.
